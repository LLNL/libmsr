...
int dump_pmc_thread_terse(FILE * writefile, const int thread)
{
    ...
    for (i = 0; i < numDevs; i++)
    {
        ...
        {
            case 8:
                fprintf(writefile, "%lu\t", *p->pmc7[i]);
            case 7:
                fprintf(writefile, "%lu\t", *p->pmc6[i]);
            case 6:
                fprintf(writefile, "%lu\t", *p->pmc5[i]);
            case 5:
                fprintf(writefile, "%lu\t", *p->pmc4[i]);
            case 4:
                fprintf(writefile, "%lu\t", *p->pmc3[i]);
            case 3:
                fprintf(writefile, "%lu\t", *p->pmc2[i]);
            case 2:
                fprintf(writefile, "%lu\t", *p->pmc1[i]);
            case 1:
                fprintf(writefile, "%lu\t", *p->pmc0[i]);
        }
    }
    return 0;
}
